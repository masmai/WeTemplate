...
function LineChartScreen({ route, navigation }) {
  ...
  return (
    <View style={{ flex: 1 }}>
      <Text >แนวโน้ม</Text>
      <Chart
        style={{ height: 270, width: 400, marginTop: 10 }}
        data={[
          { x: '2560', y: 66.78 },
          { x: '2561', y: 77.77 },
          { x: '2562', y: 90.98 },
        ...
        padding={{ left: 40, bottom: 20, right: 0, top: 20 }}
        xDomain={{ min: 2560, max: 2563 }}
        yDomain={{ min: 0, max: 100 }}
      // viewport={{ initialOrigin:{ x:20 ,y=20 } , size :{width: 15, height:15}}}
      >
        <VerticalAxis tickCount={5} theme={{ axis: { visible: false }, ticks: { visible: false }, labels: { formatter: (v) => v.toFixed(0) }, grid: { stroke: { width: 0.5 } } }} />
        ...
        <Line theme={{
          ...
          scatter: { default: { width: 10, height: 10, rx: 5, dx: 0, color: '#91b6e1' }, selected: { color: 'blue' } },
        }}
